...
func (p *Pool) put(c *Client, err error) error {
	...
	if (nil != err && (err.Error() == "connection is shut down" || strings.Contains(err.Error(), "connection reset by peer"))) || c.closing || c.shutdown || p.idle.Len() >= p.MaxIdle {
		c.Close()
		p.release()
		if nil != p.cond {
			p.cond.Signal()
...
func (pc *PoolClient) CallTimeout(serviceMethod string, args interface{}, reply interface{}, retry int, duration time.Duration) error {
	...
	for i := 0; i < retry; i++ {
		...
		if nil == err {
			break
		} else if strings.Contains(err.Error(), "connection is shut down") || strings.Contains(err.Error(), "connection reset by peer") {
			dial := pc.pool.Dial
			pc.client, err = dial()
			if nil != err {
				err = ErrShutdown
